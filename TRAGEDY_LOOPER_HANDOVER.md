...
- **`loadAndEdit(idx)` plot pre-selection — verified correct.** Loading a preloaded script into Setup for editing does correctly highlight the right main plot and subplot cards (`makePlotEl()`'s `isSel` check against `G.mainPlot`/`G.subPlots` works as designed). This was flagged "unverified" across two prior handovers; confirmed in this session by loading Script 3 ("Magical Girls' Superiority") and checking `document.querySelectorAll('#main-plot-list .plot-opt, #sub-plot-list .plot-opt')` for the `sel` class directly via JS, not just by eyeballing a screenshot — **a stale mouse cursor position can trigger CSS `:hover` styling on an unrelated card after a page navigation/scroll, which looks very similar to the real `.sel` selected-highlight in a screenshot.** If verifying selection/highlight state visually again in future, prefer checking `classList.contains('sel')` (or the equivalent state class) via `javascript_tool`/console over trusting a screenshot alone.
- **No Playwright on this Windows dev machine.** The Playwright-based testing workflow described in the previous paragraph is from a different (Linux/cloud) session environment and isn't set up here — `node` has no `playwright` module installed and there's no `/opt/pw-browsers`. On this machine, use the Browser pane tool (`mcp__Claude_Browser__*`) against a `file://` URL instead for behavioral testing; it renders a genuinely live, script-executing page (shown internally as a `data:` URL), not a static snapshot, despite what the initial `navigate` result message says.
